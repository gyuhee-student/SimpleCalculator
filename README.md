...
- 사용한 기술과 구현한 기능:
    ...
    - switch 문을 이용하여 currentOperator 변수에 따라 사칙연산을 분기 처리한다.
    - 0으로 나누기 시 예외처리를 구현하여 오류 메시지를 표시하고 상태를 초기화한다.
    - 선행 0 입력 방지 로직을 구현하여 "007"과 같은 비정상적인 숫자 입력을 차단한다.
    - isResult, isNewInput 플래그 변수를 이용하여 계산기의 입력 상태를 관리한다.
...
- 과제 내용
    - 첫번째, 두번째 사진은 연산자를 연속으로 눌러 중간 계산이 자동으로 수행되는 연속 계산 기능을 구현한 결과이다.
